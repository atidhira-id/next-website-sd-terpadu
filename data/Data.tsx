export const MainLinks = [
  { href: "/", title: "Beranda" },
  {
    href: "/tentang",
    title: "Tentang",
    child: [
      {
        href: "/",
        title: "Profil",
        description: "Sejarah, Visi Misi, dan Lambang Sekolah",
      },
      {
        href: "/pesan-kepala-sekolah",
        title: "Pesan Kepala Sekolah",
        description: "Sambutan Kepala Sekolah untuk siswa dan orang tua",
      },
      {
        href: "/guru-dan-pegawai",
        title: "Guru dan Pegawai",
        description:
          "Daftar tenaga pendidik dan staf sekolah yang berdedikasi dan berpengalaman",
      },
    ],
  },
  { href: "/berita", title: "Berita" },
  { href: "/info", title: "Info" },
];

export const newsEvents = [
  {
    type: "Berita",
    title: "Siswa SD Muhammadiyah Raih Juara 1 Lomba Cerdas Cermat",
    image: "/images/news/cerdas-cermat.jpg",
    date: "2026-02-20",
    body: "Prestasi membanggakan kembali diraih oleh siswa SD Muhammadiyah setelah berhasil meraih Juara 1 dalam Lomba Cerdas Cermat tingkat kecamatan. Kegiatan ini diikuti oleh berbagai sekolah dasar dan berlangsung dengan penuh semangat serta sportivitas.",
  },
  {
    type: "Kegiatan",
    title: "Peringatan Isra Mi'raj Nabi Muhammad SAW",
    image: "/images/news/isra-miraj.jpg",
    date: "2026-02-18",
    body: "Dalam rangka memperingati Isra Mi'raj Nabi Muhammad SAW, sekolah mengadakan kegiatan ceramah agama dan lomba islami antar kelas. Kegiatan ini bertujuan menanamkan nilai-nilai keislaman serta mempererat ukhuwah antar siswa.",
  },
  {
    type: "Berita",
    title: "Peluncuran Program Literasi Sekolah",
    image: "/images/news/literasi.jpg",
    date: "2026-02-15",
    body: "SD Muhammadiyah resmi meluncurkan Program Literasi Sekolah untuk meningkatkan minat baca siswa. Program ini melibatkan kegiatan membaca 15 menit sebelum pelajaran dimulai dan pojok baca di setiap kelas.",
  },
  {
    type: "Kegiatan",
    title: "Kunjungan Edukasi ke Museum Kota",
    image: "/images/news/museum.jpg",
    date: "2026-02-12",
    body: "Siswa kelas 5 mengikuti kunjungan edukasi ke museum kota sebagai bagian dari pembelajaran tematik. Kegiatan ini bertujuan memperluas wawasan sejarah dan budaya secara langsung.",
  },
  {
    type: "Berita",
    title: "Penerimaan Peserta Didik Baru (PPDB) Dibuka",
    image: "/images/news/ppdb.jpg",
    date: "2026-02-10",
    body: "SD Muhammadiyah membuka Penerimaan Peserta Didik Baru (PPDB) Tahun Ajaran 2026/2027. Pendaftaran dapat dilakukan secara online maupun langsung ke sekolah pada jam kerja.",
  },
  {
    type: "Kegiatan",
    title: "Class Meeting Semester Ganjil",
    image: "/images/news/class-meeting.jpg",
    date: "2026-02-05",
    body: "Setelah pelaksanaan ujian semester, sekolah mengadakan kegiatan class meeting dengan berbagai perlombaan seperti futsal, lomba mewarnai, dan permainan edukatif lainnya.",
  },
];

export const testimonials = [
  {
    name: "Rizky Pratama",
    testimony:
      "Kami sangat bersyukur menyekolahkan Rizky di SD Muhammadiyah 1 Besuki. Perkembangan akademik dan akhlaknya sangat terlihat, gurunya sabar dan perhatian sehingga Rizky semakin percaya diri dan semangat belajar setiap hari.",
  },
  {
    name: "Siti Aisyah Putri",
    testimony:
      "Sejak bersekolah di SD Muhammadiyah 1 Besuki, Aisyah menjadi lebih mandiri dan rajin beribadah. Lingkungan sekolah yang nyaman dan religius membuat kami tenang sebagai orang tua.",
  },
  {
    name: "Muhammad Alif Ramadhan",
    testimony:
      "Kami melihat perubahan positif pada Alif, baik dalam prestasi maupun sikapnya di rumah. Program pembelajaran di SD Muhammadiyah 1 Besuki sangat mendukung pengembangan karakter dan potensi anak.",
  },
  {
    name: "Nabila Zahra",
    testimony:
      "Nabila merasa senang dan betah belajar di SD Muhammadiyah 1 Besuki. Kegiatan sekolah yang beragam membuatnya lebih aktif, kreatif, dan memiliki banyak teman baik.",
  },
];
